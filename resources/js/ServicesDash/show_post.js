...
export default function Show_post(match) {
  ...
  useEffect(() => {
    axios
      .get("http://127.0.0.1:8000/api/post_s/" + match.match.params.id)
      .then((res) => {
  ...
  const [size_validation, set_size_validation] = useState({
    vin: false,
    couverture: false,
    album: false,
  });
  ...
  const HandelFile_couverture = (e) => {
    ...
    if (couverture.size > 2000000) {
      Set_Valid_Couverture({
        status: true,
        msg: "size of image is big then 3 mb",
      });
  ...
  const handelPost = () => {
    ...
    if (
      ...
      if (
        (size_validation.vin === false) &
        (size_validation.couverture === false) &
        (size_validation.album === false)
      ) {
        axios({
          method: "post",
          url:
            "http://127.0.0.1:8000/api/post_s/update/" + match.match.params.id,
          data: formdata,
        }).then((response) => {
            ...
            <table className="table ">
              ...
              <tbody>
                <div class="row">
                  ...
                  <div class="col">
                    {Valid_Couverture.status ? (
                      <div class="alert alert-danger" role="alert">
                        {Valid_Couverture.msg}
                      </div>
                    ) : (
                      <div></div>
                    )}
