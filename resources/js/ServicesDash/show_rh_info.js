...
export default function Show_rh_info(match) {
    ...
    useEffect(() => {
        axios.get('http://127.0.0.1:8000/api/rh_info/' + match.match.params.id).then(res => {
    ...
    const handelPost = () => {
        ...
        if (title !== '' & content !== '' & vin !== '' & couverture !== '') {
            ...
            if (size_validation.vin === false & size_validation.couverture === false & size_validation.album === false) {
                ...
                axios({
                    method: 'post',
                    url: 'http://127.0.0.1:8000/api/rh_info/update/' + match.match.params.id,
                    data: formdata
